...
const DocumentViewer: React.FC<DocumentViewerProps> = ({
  ...
  const fileUrl = document.file_path ? fileStorage.getPublicUrl('marches', document.file_path) : null;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const fileExtension = document.file_path.split('.').pop()?.toLowerCase() || '';
      const finalFilename = originalFilename.includes(`.${fileExtension}`) ? originalFilename : `${originalFilename}.${fileExtension}`;
  ...
  return <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="mt-4">
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="apercu" className="mt-4">
                {isFileChecking || isLoadingPdf ? <div className="flex flex-col items-center justify-center h-[70vh] bg-gray-50 rounded">
                    <Loader2 className="h-12 w-12 text-gray-400 animate-spin mb-4" />
                    <p className="text-gray-600">
                      {isLoadingPdf ? "Préparation du fichier PDF..." : "Vérification du fichier..."}
                    </p>
                  </div> : fileUrl && !fileError ? <div className="space-y-4">
                    <div className="flex justify-between">
                      <Button variant="outline" onClick={() => window.open(fileUrl, '_blank')} className="flex items-center gap-2">
                    ...
                    {fileError ? <Alert variant="destructive" className="mb-4">
                  ...
                  <iframe src={pdfDataUrl} className="w-full h-[70vh] border rounded" title={document.nom} onError={e => {
                    console.error("Error loading PDF in iframe:", e);
                    setFileError("Impossible d'afficher le fichier PDF. Essayez de le télécharger.");
                  }} /> :
                  // Fallback to regular iframe with URL
                  <iframe src={`${fileUrl}#view=FitH`} className="w-full h-[70vh] border rounded" title={document.nom} onError={e => {
                    console.error("Error loading PDF in iframe:", e);
                    setFileError("Impossible d'afficher le fichier PDF. Essayez de le télécharger.");
                  }} />}
                      ...
                      </> : fileType === 'image' ? <img src={fileUrl || ''} alt={document.nom} className="max-w-full mx-auto max-h-[70vh] object-contain" onError={() => {
                  ...
                  setFileError("Impossible d'afficher l'image. Essayez de la télécharger.");
                        ...
                        <Button variant="outline" onClick={handleDownload} className="mt-4 flex items-center gap-2">
                          <Download className="h-4 w-4" />
                          Télécharger
                        </Button>
                      </div>}
                  </div> : <div className="flex flex-col items-center justify-center p-8 bg-gray-50 rounded border border-dashed">
                    <AlertCircle className="h-12 w-12 text-amber-500 mb-4" />
                    <h3 className="text-lg font-semibold mb-2">
                      {fileError || "Aucun fichier associé"}
                    </h3>
                    <p className="text-gray-500 text-center mb-4">
                      {fileError ? "Un problème est survenu avec le fichier associé à ce document." : "Ce document n'a pas de fichier associé. Vous pouvez en télécharger un maintenant."}
                    </p>
                    
                    {isMandataire && <Button variant="outline" onClick={() => setIsUploaderOpen(true)} className="flex items-center gap-2">
              ...
              <TabsContent value="details" className="mt-4">
                <DocumentDetails document={document} formatDate={formatDate} onDocumentUpdated={handleDocumentUpdate} isMandataire={isMandataire} // Pass isMandataire to DocumentDetails
              />
              </TabsContent>
              
              <TabsContent value="versions" className="mt-4">
                <DocumentVersions document={document} onVersionAdded={handleDocumentUpdate} isMandataire={isMandataire} // Pass isMandataire to DocumentVersions
              />
              </TabsContent>
          ...
          {isMandataire && isDocumentEditable(document) && <div className="flex justify-end gap-2 mt-4">
              {document.file_path || fileError}
              
              <ModifyDocumentButton document={document} onDocumentUpdated={handleDocumentUpdate} isMandataire={isMandataire} />
            </div>}
        </DialogContent>
      </Dialog>
      
      {document && <DocumentUploader documentId={document.id} open={isUploaderOpen} onOpenChange={setIsUploaderOpen} onSuccess={handleUploadSuccess} isMandataire={isMandataire} />}
    </>;
};
