...
const MarcheStatsCard: React.FC<MarcheStatsCardProps> = ({ documentStats }) => {
  // Calculer les questions ouvertes (exemple)
  const questionsOuvertes = 0; // Cette valeur pourrait être passée via les props dans le futur
    ...
    <Card className="p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-gray-50 p-4 rounded-md text-center">
          <span className="block text-3xl font-bold text-gray-800">{documentStats.total || 0}</span>
          <span className="text-sm text-gray-500">Documents Total</span>
        </div>
        <div className="bg-gray-50 p-4 rounded-md text-center">
          <span className="block text-3xl font-bold text-green-500">{documentStats.approuves || 0}</span>
          <span className="text-sm text-gray-500">Documents Approuvés</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-blue-50 p-4 rounded-md text-center">
          <span className="block text-3xl font-bold text-blue-500">{documentStats.enAttente || 0}</span>
