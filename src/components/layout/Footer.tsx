...
import { Mail, Phone, MapPin, FileText, Shield, BookOpen } from 'lucide-react'

export default function Footer() {
  return (
    <footer className="bg-btp-navy text-white pt-12 pb-6">
      <div className="container-custom">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-start space-x-2 text-sm text-gray-300">
                ...
                <span>12 rue des Marchés Publics<br />75001 Paris, France</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="font-heading font-semibold text-lg mb-4">Fonctionnalités</h4>
            <ul className="space-y-2">
              {[
                ...
                { label: 'Communication & workflow', href: '/fonctionnalites/communication' },
                { label: 'Tableaux de bord', href: '/fonctionnalites/tableaux-bord' },
              ].map((item, index) => (
                <li key={index}>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[
                { label: 'Centre d\'aide', href: '/support', icon: <BookOpen className="h-4 w-4" /> },
                { label: 'Blog BTP', href: '/blog', icon: <FileText className="h-4 w-4" /> },
                { label: 'Conformité RGPD', href: '/confidentialite', icon: <Shield className="h-4 w-4" /> },
                { label: 'CCAG Travaux 2021', href: '/ccag-travaux', icon: <FileText className="h-4 w-4" /> },
                { label: 'Témoignages clients', href: '/temoignages' },
                { label: 'Partenaires', href: '/partenaires' },
              ].map((item, index) => (
                <li key={index}>
                  <Link to={item.href} className="text-sm text-gray-300 hover:text-white transition-colors flex items-center space-x-2">
                    {item.icon}
          ...
          <div>
            <h4 className="font-heading font-semibold text-lg mb-4">Restez informé</h4>
            ...
            <form className="space-y-2">
              <input 
                ...
                className="w-full px-3 py-2 bg-btp-navy border border-gray-600 rounded text-white placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-btp-blue"
              />
              <Button variant="btpPrimary" className="w-full">S'inscrire</Button>
            </form>
        ...
        <div className="border-t border-gray-700 pt-6 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex space-x-6">
            ...
            <Link to="/cgv" className="text-sm text-gray-400 hover:text-white transition-colors">CGV</Link>
